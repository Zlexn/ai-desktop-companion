...
- Stage 3A–3H：长期记忆 foundation、候选确认、相关性检索、冲突审计、保守语义冲突检测、opt-in embedding retrieval、embedding smoke、隔离真实 embedding 模型评估均已完成；证据记录于 `docs/stage3-memory-foundation.md`、`docs/stage3b-memory-candidate-confirmation.md`、`docs/stage3c-memory-relevance-retrieval.md`、`docs/stage3d-memory-conflict-audit.md`、`docs/stage3e-semantic-conflict-detection.md`、`docs/stage3f-memory-embedding-retrieval.md`、`docs/stage3g-real-embedding-smoke.md`、`docs/stage3h-real-embedding-model-evaluation.md`。
- Stage 3I（下一步）：User-Confirmed LLM Memory Candidate Extraction；新增 opt-in LLM 候选抽取路径，默认保持 heuristic，候选仍必须用户确认后才可进入 active 长期记忆。

未实现范围：

- 真实 TTS 生产化打包与完整资源驻留策略。
- 阶段 3：3I LLM-based 用户确认式记忆候选抽取、会话摘要独立存储设计、真实 embedding 模型生产选型、自动冲突合并/解决工作流。
- 阶段 4：情感系统。

聊天历史在阶段 1 中仅作为会话消息保存，不视为长期记忆。

### Stage 3 next task: 3I User-Confirmed LLM Memory Candidate Extraction

The next minimal complete loop is Stage 3I. It should add an opt-in LLM candidate extraction path beside the existing heuristic candidate extractor:

```env
MEMORY_CANDIDATE_PROVIDER=heuristic
# or explicitly opt in after implementation:
MEMORY_CANDIDATE_PROVIDER=llm
```

Required boundaries for 3I:

- Default behavior remains heuristic until LLM extraction is explicitly enabled.
- The extractor may process only the current user message, not old chat history or assistant messages.
- LLM output may create only `pending` memory candidates; the user must confirm before a candidate becomes an active long-term memory.
- Pending/dismissed candidates must not be injected into chat context.
- Extraction failure must not break normal chat.
- 3I must not implement session summaries, automatic active memory writes, automatic conflict merge/resolve, or Stage 4 emotion state.

Design reference: `docs/superpowers/specs/2026-07-08-stage-3i-llm-memory-candidate-extraction-design.md`.
